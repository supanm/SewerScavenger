...
namespace SewerScavenger.Models
{
    public class Character : Entity<Item>
    {
        ...
        {
            { 0, 0, 0, 0 }, { 0, 1, 1, 1}, { 300, 1, 2, 1 }, { 900, 2, 3, 1 },
        ...
        public void AddExperience(int xp)
        {
            ...
            while (continueLeveling)
            {
                ...
                // Removes the old level stat boost and adds the new level stat boost
                if (Level < 20 && XP > LevelUpTable[Level+1, 0])
                {
                    Level++;
        ...
        public void SetLevel(int level)
        {
            ...
            Defense += LevelUpTable[Level, 2];
            Speed += LevelUpTable[Level, 3];
            XP = LevelUpTable[level, 0] + 1; // +1 so that the character does not level up to current level. 
        }
    }
}